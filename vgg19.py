...
def train_vgg19(dataset_path, model_path, epochs, ortho=False, ortho_factor=0.1):
    images = []
    labels = []
    for dataset_file in os.listdir(dataset_path + "/train"):
        with open(dataset_path + "/train/" + dataset_file, "rb") as f:
            d = pickle.load(f)
    ...
    normalization_shift = np.mean(images, axis=(0, 1, 2))
    normalization_factor = np.std(images, axis=(0, 1, 2))
    normalization_factor[-1] = 1.0
    normalization_factor *= 1.0 / 255.0

    images += normalization_shift
    images /= normalization_factor

    hist = {}

    ae = VGG19(input_shape=(img_dim, img_dim, 3), ortho_regularizer=ortho, ortho_strength=ortho_factor)
    # if os.path.isfile(model_path):
    #     ae.load_model(path=model_path)

    hist = ae.train(epochs, data=(images, labels), batch_size=32)
...
if __name__ == '__main__':
    try:
        ...
        if args.train:
            hist = train_vgg19(args.dataset, args.model, args.epochs, args.ortho_regularization, args.ortho_factor)
            if hist:
